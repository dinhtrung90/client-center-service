...
/**
 * Performance test for the EmployerBrand entity.
 */
class EmployerBrandGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the EmployerBrand entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all employerBrands")
            .get("/services/clientcenterservice/api/employer-brands")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new employerBrand")
            .post("/services/clientcenterservice/api/employer-brands")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "logoUrl":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_employerBrand_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created employerBrand")
                .get("/services/clientcenterservice${new_employerBrand_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created employerBrand")
            .delete("/services/clientcenterservice${new_employerBrand_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
