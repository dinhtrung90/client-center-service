...
/**
 * Performance test for the UserAddress entity.
 */
class UserAddressGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the UserAddress entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all userAddresses")
            .get("/services/clientcenterservice/api/user-addresses")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new userAddress")
            .post("/services/clientcenterservice/api/user-addresses")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_userAddress_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created userAddress")
                .get("/services/clientcenterservice${new_userAddress_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created userAddress")
            .delete("/services/clientcenterservice${new_userAddress_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
